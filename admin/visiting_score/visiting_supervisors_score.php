  ...
  if($search_by!=""&& $search_term!=""){
    ...
    switch ($search_by) {
      ...
      case 'Last Name':
        ...
        break;
      case 'Index Number':
        $mysql_query_1 = "SELECT * FROM industrial_registration WHERE index_number LIKE '%$search_term%'";
        break;
      case 'Programme':
        $mysql_query_1 = "SELECT * FROM industrial_registration WHERE programme LIKE '%$search_term%'";
        break;
      case 'Level':
        $mysql_query_1 = "SELECT * FROM industrial_registration WHERE level LIKE '%$search_term%'";
        break;
      case 'Session':
        $mysql_query_1 = "SELECT * FROM industrial_registration WHERE session LIKE '%$search_term%'";
        break;
      case 'Score':
        $mysql_query_1 = "SELECT * FROM industrial_registration WHERE visiting_supervisor_grade LIKE '%$search_term%'";
        break;
      default:
        $mysql_query_1 = "SELECT * FROM industrial_registration";
        break;
                  ...
                  <tbody>
                    ...
                    if ($execute_result_query) {
                      while ($row_set = mysqli_fetch_array($execute_result_query)){
                        ...
                        echo "<td>".htmlspecialchars($row_set["first_name"]." ".$row_set["last_name"])."</td>";
                        echo "<td>".htmlspecialchars($row_set["index_number"])."</td>";
                        echo "<td>".htmlspecialchars($row_set["programme"])."</td>";
                        echo "<td>".htmlspecialchars($row_set["level"])."</td>";
                        echo "<td>".htmlspecialchars($row_set["session"])."</td>";
                        echo "<td>Industrial Registration</td>";
                        echo "<td>".htmlspecialchars($row_set["visiting_supervisor_name"] ?? '')."</td>";
                        echo "<td>".htmlspecialchars($row_set["visiting_supervisor_contact"] ?? '')."</td>";
                        echo "<td>".htmlspecialchars($row_set["visiting_supervisor_grade"] ?? '')."</td>";
                        echo "</tr>";
                      }
                    }
                    ?>
